...
J. Iverson, C. Kamath, and G. Karypis, “Evaluation of connected-component labeling algorithms
 for distributed-memory systems,” Parallel Computing, vol. 44, pp. 53–68, May 2015, doi: 10.1016/j.parco.2015.02.005.

“Reachability queries, which ask whether there exists a path between two vertices in a directed graph, 
are fundamental operations in graph databases and are widely used in applications such 
as XML querying, program analysis, social networks, and biological networks.
Computing the transitive closure is a classical approach to answering reachability queries.”
...
The input graphs are sparse, meaning the number of edges < number of vertex pairs. When represented as matrices, the computation
only goes over the nonzero entries. This allows SCC to operate near linear time scaling with the number of edges, and limits the work done
during the transitive closure algorithm.
...
def _transitive_closure_computation(xp, edges_b):
    ...
    for iteration in range(max_iterations):
        graph = xp.lazy(graph)
        nextGraph = xp.compute(xp.einsum("nextGraph[i,j] |= graph[i,k] & graph[k,j]", graph=graph))
